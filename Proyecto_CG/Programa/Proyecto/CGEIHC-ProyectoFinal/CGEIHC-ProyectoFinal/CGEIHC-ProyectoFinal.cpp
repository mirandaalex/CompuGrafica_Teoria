...
float velocidadPuerta = 0.4f ;
float velocidadVentanas = 0.02f ;
float velocidadTapaInodoro = 0.4f ;
float velocidadGlobo = 0.05f ;
...
#define MAX_RIGGING_BONES 100
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//TEXTO
		{
			renderText(textShader, "Camara modo: "+std::to_string(cameraMode), 10.0f, 10.0f, 0.5f, glm::vec3(0.1f, 0.1f, 0.1f),projection);
			zoom=45.0f - camera.Zoom;
			renderText(textShader, "zoom: "+std::to_string(int(zoom)), 750.0f, 10.0f, 0.5f, glm::vec3(0.1, 0.1f, 0.1f), projection);
		}
		glUseProgram(0);
		{
			ufoAngle += 4;
...
void processInput(GLFWwindow* window)
{
	...
	{
		rotacionTapaInodoro -= velocidadTapaInodoro;
	...
	{
		traslacionVentanas -= velocidadVentanas;
	...
	{
		escalamientoGlobo -= velocidadGlobo;
